...
def _realtime_classroom_instructions(child, lesson, active_day, state) -> str:
    ...
    return (
        ...
        "Ask whether the learner is ready only once, during the opening greeting. After that, never use another "
        "readiness question as a handoff. Never ask the learner to find, fetch, hold up, or describe objects around "
        "them; teach from the authored examples already displayed in the lesson note. Never end a turn with only "
        "'let's keep going', 'let's practise more', or a similar transition. "
        ...
        f"The learner's name is {child.name}. Warmly greet them by name, then use their name naturally and "
        "regularly in encouragement and transitions without repeating it in every sentence. "
